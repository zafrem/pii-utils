// Command aws-s3-grep scans objects under an S3 prefix for PII using the
...
func run() error {
	var o options
	fs := flag.NewFlagSet("aws-s3-grep", flag.ContinueOnError)
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: aws-s3-grep [flags] s3://bucket[/prefix]\n\nFlags:\n")
		fs.PrintDefaults()
	}
	...
	if o.estimateOnly {
		est := cost.Compute(inv, own.SameAccount)
	...
	est := cost.Compute(cost.Inventory{Objects: int64(len(remaining)), Bytes: remainingBytes, ListRequests: inv.ListRequests}, own.SameAccount)
