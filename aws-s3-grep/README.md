...
Scan objects under an S3 prefix for personally identifiable information (PII)
using the [pii-pattern-engine](https://github.com/zafrem/pii-pattern-engine)
...
```bash
aws-s3-grep --ner s3://my-bucket/logs/
```
...
aws-s3-grep --estimate-only s3://my-bucket/logs/

# Scan, throttled to 20 req/s, write a JSON report
aws-s3-grep --rps 20 --concurrency 8 --json findings.json s3://my-bucket/logs/

# Only Korean + common credit-card rules, hard-stop on cross-account
aws-s3-grep --locations kr,comm --categories credit_card \
  --require-same-account s3://my-bucket/

# Non-interactive (CI): skip the large-scan prompt
aws-s3-grep --yes s3://my-bucket/
```
